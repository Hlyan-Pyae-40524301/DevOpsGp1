...
//        CountryLanguage lag = a.getLanguage(5.3F);
        ...
        // Extract population information
        ArrayList<Country> country10 = a.getPopulationOfPeopleInAContinent();
        ...
        // Extract population information
        ArrayList<City> city17 = a.getPopulationOfPeopleInACity();
...
//    }
    ...
    /** Gets the current City and Country.
     * @return A list of population of a continent, or null if there is an error.
     */
    public ArrayList<Country> getPopulationOfPeopleInAContinent()
    {
        try
        {
            // Create an SQL statement
            ...
            String strSelect =
                    "SELECT country.Continent, " +
                            "SUM(country.Population) AS total_population_of_Asia, " +
                            "SUM(coalesce(city.Population, 0)) AS people_living_in_cities, " +
                            "(SUM(coalesce(city.Population, 0)) / SUM(country.Population)) * 100 AS percentage_people_living_in_cities, " +
                            "SUM(country.Population - coalesce(city.Population, 0)) AS people_not_living_in_cities, " +
                            "((SUM(country.Population) - SUM(coalesce(city.Population, 0))) / SUM(country.Population)) * 100 AS percentage_people_not_living_in_cities " +
            ...
            while (rset.next())
            {
                Country cou = new Country();
                cou.Continent = rset.getString("country.Continent");
                cou.TotalPopulation = rset.getLong("total_population_of_Asia");
                cou.PeopleLivingInCities = rset.getLong("people_living_in_cities");
                cou.PercentagePeopleLivingInCities = rset.getFloat("percentage_people_living_in_cities");
                cou.PeopleNotLivingInCities = rset.getLong("people_not_living_in_cities");
                cou.PercentagePeopleNotLivingInCities = rset.getFloat("percentage_people_not_living_in_cities");
    ...
    /**
     * Prints a list of Capital Cities.
     * @param country10 The list of city to print.
     */
    public void printPopulationOfPeopleInAContinent(ArrayList<Country> country10)
    {
        // Check country is not null
        if (country10 == null)
        {
            System.out.println("No Population Of People In A Continent (Asia)");
            return;
        }

        // Title
        System.out.println("Population Of People In A Continent (Asia) Report");

        // Print header
        System.out.println(String.format("%-20s %-20s %-20s %-20s %-20s %-20s", "ContinentName", "TotalPopulation", "PeopleInCity", "PeopleInCity(%)", "PeopleNotInCity", "PeopleNotInCity(%)"));
        // Loop over capital cities in the list
        for (Country cou : country10)
        {
            if (cou == null)
                continue;
            String cou_string =
    ...
    /** Gets the current City and Country.
     * @return A list of population of people in city, or null if there is an error.
     */
    public ArrayList<City> getPopulationOfPeopleInACity()
    {
        try
        {
            // Create an SQL statement
            ...
            // Create string for SQL statement
            String strSelect =
                    "SELECT SUM(city.Population) AS Total_Population_of_Mandalay " +
                            "FROM city " +
                            "WHERE city.Name='Mandalay';";
            // Execute SQL statement
            ResultSet rset = stmt.executeQuery(strSelect);
            // Extract Country information
            ...
            {
                City cit = new City();
                cit.TotalPopulation = rset.getLong("Total_Population_of_Mandalay");
                city17.add(cit);
            }
            return city17;
        }
    ...
    /**
     * Prints a list of Capital Cities.
     * @param city17 The list of city to print.
     */
    public void printPopulationOfPeopleInACity(ArrayList<City> city17)
    {
        // Check City Population is not null
        if (city17 == null)
        {
            System.out.println("No Population Of People In A City (Mandalay)");
            return;
        }

        // Title
        System.out.println("Population Of People In A City (Mandalay) Report");
